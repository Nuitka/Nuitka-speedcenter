...
POSTS = (
    ( "constructs/*.rst", "constructs", "post.tmpl"),
)
PAGES = (
    ( "index/*.rst", ".", "page.tmpl" ),
...
def hackContentClasses(infile):
    import os

    os.system(
        "sed -i '%s' -e 's/contents topic/contents pull-right navbar alert topic/'" % infile
    )
...
def htmlcompressor(infile):
    ...
    return filters.runinplace(r'java -jar %s --preserve-line-breaks --nomunge --compress-js %%1 -o %%2' % os.path.join( os.path.dirname( __file__ ), "tools/htmlcompressor-1.5.3.jar" ), infile )
